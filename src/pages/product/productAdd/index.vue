    ...
    <Card :bordered="false" dis-hover class="ivu-mt">
      ...
      >
        <Row :gutter="24" type="flex" v-show="currentTab === '1'">
		  ...
		  <Col span="24">
		    <FormItem label="款号：" prop="kh_code">
		      <Input
		        v-model="formValidate.kh_code"
		        placeholder="请输入款号"
		        class="perW50"
		      />
		    </FormItem>
		  ...
		  <Col span="24">
		    <FormItem label="基础工费：" prop="fee_jcgj">
		      <Input
		        v-model="formValidate.fee_jcgj"
		        placeholder="请输入基础工费"
		        class="perW50"
		      />
		    </FormItem>
		  ...
		  <Col span="24">
		    <FormItem label="附加费：" prop="fee_fjf">
		      <Input
		        v-model="formValidate.fee_fjf"
		        placeholder="请输入附加费"
		        class="perW50"
		      />
		    </FormItem>
		  ...
		  <Col span="24" v-for="(item,index) in catelSelect" :key="index">
		    <FormItem :label="item.name+'：'">
				<RadioGroup v-if="item.sel<1" v-model="formValidate[item.field]" type="button" button-style="solid">
					<Radio :label="son.id" v-for="(son,sonsindex) in item.sons" :key="sonsindex">{{son.name}}</Radio>
				</RadioGroup>
				<CheckboxGroup v-else v-model="formValidate[item.field]">
					<Checkbox :label="son.id" v-for="(son,sonsindex) in item.sons" :key="sonsindex" border>{{son.name}}</Checkbox>
				</CheckboxGroup>
		    </FormItem>
		  </Col>
        </Row>
          ...
          <Col span="24">
            <FormItem label="商品详情：">
              <vue-ueditor-wrap
                ...
                :config="myConfig"
                style="width: 90%;line-height: 23px;"
              ></vue-ueditor-wrap>
            </FormItem>
          </Col>
        </Row>
...
export default {
  ...
  data() {
    return {
      ...
      ruleValidate: {
		...
		fee_jcgj: [
		  { required: true, message: "请输入基础工费", trigger: "blur" },
		],
		fee_fjf: [
  ...
  methods: {
    ...
    // 立即生成
    generate(type) {
      generateAttrApi({ attrs: this.attrs }, this.formValidate.id, type)
        .then((res) => {
    ...
    getInfo() {
      ...
      productInfoApi(that.$route.params.id)
        .then(async (res) => {
		  ...
		  if(data.s_tazx){
			  let list = []
			  data.s_tazx.split(',').forEach(item=>{
				  list.push(Number(item))
			  })
			  data.s_tazx = list
		  }else{
    ...
    handleSubmit(name) {
      this.$refs[name].validate((valid) => {
        if (valid) {
		  ...
		  data.s_gy = data.s_gy.length>0?data.s_gy.join(','):'';
		  data.s_ks = data.s_ks.length>0?data.s_ks.join(','):'';
		  data.s_tazx = data.s_tazx.length>0?data.s_tazx.join(','):'';
		  data.s_ys = data.s_ys.length>0?data.s_ys.join(','):'';
		  data.s_zt = data.s_zt.length>0?data.s_zt.join(','):'';
          productAddApi(data)
            .then(async (res) => {
              this.$Message.success(res.msg);
              setTimeout(() => {
